...
function onloadCalls(type){
	if(type == "onload"){
		...
		for(var i in globalDepartmentId){
			...
			}else if(globalDepartmentId[i] == 19){
				/* $("#subDepartmentId").append('<option value="19">IT E& C DEPARTMENT</option>')
				$("#departmentId").append('<option value="19">IT E& C DEPARTMENT</option>') */
			}else if(globalDepartmentId[i] == 20){
				/* $("#subDepartmentId").append('<option value="48">PANCHAYAT RAJ - COMMISSIONER</option>')
				$("#subDepartmentId").append('<option value="48">PANCHAYAT RAJ -  ENC</option>')
				$("#subDepartmentId").append('<option value="48">LED MAINTENANCE DEPARTMENT</option>')
				$("#departmentId").append('<option value="20">PANCHAYAT RAJ</option>') */
			}else if(globalDepartmentId[i] == 48){
				/* $("#subDepartmentId").append('<option value="48">RURAL DEVELOPMENT DEPARTMENT</option>')
				$("#departmentId").append('<option value="48">RURAL DEVELOPMENT DEPARTMENT</option>') */
			}else if(globalDepartmentId[i] == 49){
				/* $("#subDepartmentId").append('<option value="49">RURAL WATER SUPPLY</option>')
				$("#departmentId").append('<option value="49">RURAL WATER SUPPLY</option>') */
			}
		}
		$("#departmentId").trigger("chosen:updated")
	}
	getJalavaniDashBoardOverview(type);
...
function getJalavaniDashBoardOverview(type){
	$("#jalavaniTabOverVewDivId").html(spinner);
	globalRDUId = globalDepartmentId[0];
	if(globalDepartmentId[0] == 48){
		globalDepartmentId = [65]; 
	}else if(globalDepartmentId[0] == 0){
		globalDepartmentId = [19,20,49,65,67,68]; 
	}
	var json={
		fromDateStr:currentFromDate,
		toDateStr:currentToDate,
		deptIdList:globalDepartmentId
	}
	$.ajax({                
	type:'POST',    
	...
	beforeSend :   function(xhr){
		...
		if(result !=null){
			buildJalavaniDashBoardOverview(result,type);
		}
	});
}
function buildJalavaniDashBoardOverview(result,type){
	var str='';
	var str1='';
	var str2='';
	...
	for(var i in result.subDeptList){
		if(globalRDUId == 48){
			if(i != 0){
				glblDeptIdArray.push(result.subDeptList[i].statusId);			
			}
		}
	}
	
	if(type=="onload"){
		for(var i in result.prntDeptList){
			str1+='<option value="'+result.prntDeptList[i].statusId+'" selected>'+result.prntDeptList[i].status+'</option>';
			if(result.prntDeptList[i].statusId == 48){
					str2+='<option value="0">All</option>';		
			}
		}
		for(var i in result.subDeptList){
			if(result.subDeptList[i].statusId != 73){
				//if(result.subDeptList[i].alertTypeId == 72 ){
					if( result.subDeptList[i].statusId == globalDepartmentId[0] ){
						str2+='<option value="'+result.subDeptList[i].statusId+'" selected >'+result.subDeptList[i].status+'</option>';
					}else {
						str2+='<option value="'+result.subDeptList[i].statusId+'" >'+result.subDeptList[i].status+'</option>';	
					}	
				//}
			}
		}
	...
	for(var i in result.subList2){
		//if(result.subList2[i].percentage !=null && result.subList2[i].percentage>0){
			...
			if(result.subList2[i].percentage > 0){
				mainArr.push({y:parseFloat(result.subList2[i].percentage),"extra":result.subList2[i].monthCount})
			}
		//}
		/* else{
...
function getJalavanilocationAndStatusDetailsInfo(type,alertCategoryId,searchType){
	...
	var temp="";
	//var alertCode='MM'+","+'SI'+","+'CF'+","+'OT'+","+'NS'+","+'NI'+","+'LV'+","+'HV'+","+'LO';
	if(globalDepartmentId != null && globalDepartmentId.length > 0){
      for(var i in globalDepartmentId){
        temp=i==0?globalDepartmentId[i]:temp+","+globalDepartmentId[i];
      }
    }
	$.ajax({
		...
		//url: "http://192.168.11.193:8089/PartyAnalyst/WebService/getJalavanilocationAndStatusDetailsInfo/"+currentFromDate+"/"+currentToDate+"/"+searchType+"/"+type+"/"+alertCategoryId+"/"+temp
		
		//url: "http://192.168.11.193:8089/PartyAnalyst/WebService/getLEDLocationAndStatusDetailsInfo/"+currentFromDate+"/"+currentToDate	+	"/"+searchType+"/"+type+"/"+alertCategoryId+"/"+temp+"/"+'N'+"/"+alertCode
...
function getJalavaniAlertSourceDetailsInformation(alertCount,categoryId,statusid,locationValueName,locationValue,statusName,statusType){
	...
	$.ajax({
		...
		//url: "http://192.168.11.193:8089/PartyAnalyst/WebService/getJalavaniAlertSourceDetailsInformation/"+currentFromDate+"/"+currentToDate+"/"+statusType+"/"+locationValueName+"/"+locationValue+"/"+categoryId+"/"+statusid+"/"+globalDepartmentId
	}).then(function(result){
		 if(result != null && result.length > 0){
...
function buildAlertDtlsBasedOnStatusClick(result,statusName,statuscount,blockType,jsObj)
{
	var str='';
	var alertId = '';
	...
	//if(blockType != "closedAndReopend"){
		...
		if (jsObj.minValue == 0) { 
				...
				getJalavaniAlertForClosedAndReopenDetails(jsObj.statusId,jsObj.statusName,jsObj.categoryId,jsObj.minValue,jsObj.maxValue);
				}
			});
		}
	}**/
...
function viewAlertHistory(alertId){
	...
	$.ajax({
		...
		//url: "http://192.168.11.193:8089/PartyAnalyst/WebService/viewAlertHistory/"+alertId+"/task"
	}).then(function(result){
		if(result != null && result.length> 0)
		{
			alertHistory(result);
...
function getStatusCompletionInfo(alertId){
	...
	$.ajax({
		...
		//url: "http://192.168.11.193:8089/PartyAnalyst/WebService/getStatusCompletionInfo/"+alertId+"/0/0/0/0/0"
	}).then(function(result){
...
function getAlertData(alertId){
	...
	$.ajax({
		...
		//url: "http://192.168.11.193:8089/PartyAnalyst/WebService/getAlertData/"+alertId
	}).then(function(result){
		$("#alertDetails").html('');
...
function getAlertCategoryByAlert(alertId){
	...
	$.ajax({
		...
		//url: "http://192.168.11.193:8089/PartyAnalyst/WebService/getAlertCategoryByAlert/"+alertId
	}).then(function(result){
		if(result != null)
		{
			var str='';
...
function getDocumentsForAlerts(alertId){
	...
	$.ajax({
		...
		//url: "http://192.168.11.193:8089/PartyAnalyst/WebService/getDocumentsForAlerts/"+alertId
	}).then(function(result){
		$("#existingDocsDivId").html('');
		if(result != null && result.length > 0){
			var str='';
...
function getDepartmentsByAlert(alertId){
	...
	$.ajax({
		...
		//url: "http://192.168.11.193:8089/PartyAnalyst/WebService/getDepartmentsByAlert/"+alertId
	}).then(function(result){
		var str='';
		str+='<p class="m_top20">';
			for(var i in result)
...
function getAssignedOfficersDetails(alertId){
	...
	$.ajax({
		...
		//url: "http://192.168.11.193:8089/PartyAnalyst/WebService/getAssignedOfficersDetails/"+alertId
	}).then(function(result){
		if(result != null && result.length > 0)
		{
...
function getTotalArticledetails(articleId){
	...
	$.ajax({
		  type : 'GET',      
			...
			//url: "http://192.168.11.147:8080/CommunityNewsPortal/webservice/getArticlesFullDetails/"+articleId+""
...
function getJalavaniFeedBackDetailsInfo(){
	...
	$.ajax({
		...
		//url: "http://192.168.11.193:8089/PartyAnalyst/WebService/getJalavaniFeedBackDetailsInfo/"+currentFromDate+"/"+currentToDate
	}).then(function(result){
		if(result !=null && result.length>0){
...
function getJalavaniFeedBackNotSatisifiedAlertsInfo(feedBackStatusId,districtId,statusName,alertCount){
	...
	$.ajax({
		...
		//url: "http://192.168.11.193:8089/PartyAnalyst/WebService/getJalavaniFeedBackNotSatisifiedAlertsInfo/"+currentFromDate+"/"+currentToDate+"/"+feedBackStatusId+"/"+districtId
	}).then(function(result){
		 if(result != null && result.length > 0){
			buildAlertDtlsBasedOnStatusClick(result,statusName,alertCount,"");
		}else{
...
$(document).on("click",".closedReopenAlertsCls",function(){
	...
	getJalavaniAlertForClosedAndReopenDetails(statusId,statusName,categoryId,0);
	$("#alertManagementPopup").modal({
		show: true,
		keyboard: false,
		backdrop: 'static'
...
function getJalavaniAlertForClosedAndReopenDetails(statusId,statusName,categoryId,minValue){
	//$('#timeSeriesWisGraphDivId').html(spinner);
	$("#modalHeadingTotal").html("");
	var json=-{
		statusId:statusId,
	...
	$.ajax({
		url: wurl+"/WebService/getJalavaniAlertForClosedAndReopenDetails/"+currentFromDate+"/"+currentToDate+"/"+statusId+"/"+globalDepartmentId+"/"+categoryId+"/"+1+"/"+0
		//url: "http://192.168.11.193:8089/PartyAnalyst/WebService/getJalavaniAlertForClosedAndReopenDetails/"+currentFromDate+"/"+currentToDate+"/"+statusId+"/"+globalDepartmentId+"/"+categoryId+"/"+1+"/"+0
...
function getJalavaniIvrWiseSummaryGraphDetailsInfo(divId){
	...
	$.ajax({
		...
		//url: "http://192.168.11.193:8089/PartyAnalyst/WebService/getJalavaniIvrWiseSummaryGraphDetailsInfo/"+currentFromDate+"/"+currentToDate
	}).then(function(result){
		if(result != null && result.length > 0){
			//buildJalavaniStatusWiseSummaryGraphDetailsInfo(result,divId);
...
function getJalavaniIvrRespondantsGraphDetailsInfo(divId){
	...
	$.ajax({
		...
		//url: "http://192.168.11.193:8089/PartyAnalyst/WebService/getJalavaniIvrRespondantsGraphDetailsInfo/"+currentFromDate+"/"+currentToDate
	}).then(function(result){
		if(result != null && result.length > 0){
...
function getJalavaniIvrSummaryWiseClick(statusId,probTypeId,statusName,districtId,satisfiedStatus,alertCount){
	...
	$.ajax({
		...
		//url: "http://192.168.11.193:8089/PartyAnalyst/WebService/getJalavaniIvrSummaryWiseClick/"+currentFromDate+"/"+currentToDate+"/"+statusId+"/"+probTypeId+"/"+districtId+"/"+satisfiedStatus
	}).then(function(result){
		if(result != null && result.length > 0){
			buildAlertDtlsBasedOnStatusClick(result,statusName,alertCount,"","");
		}else{
...
$(document).on("change","#subDeptId",function(e){
	globalDepartmentId=[];
	if($(this).val()==0){
		globalDepartmentId = glblDeptIdArray;
	}else{
		globalDepartmentId.push($('#subDeptId option:selected').val());
	}
	//$("#alertTypeId").val(0);
	//$("#alertTypeId").trigger("chosen:updated")
	onloadCalls("change");
