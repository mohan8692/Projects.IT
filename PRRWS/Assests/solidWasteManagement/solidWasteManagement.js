...
var levelWiseOverviewArr = ['state','district','constituency','mandal','panchayat'];
...
function levelWiseOverview()
{
				...
				if(i == 0)
				{
					...
					collapse+='<div class="panel-body">';
						collapse+='<div class="row">';
							if(levelWiseOverviewArr[i] == 'constituency' || levelWiseOverviewArr[i] == 'mandal' || levelWiseOverviewArr[i] == 'panchayat')
							{
							...
							if(levelWiseOverviewArr[i] == 'mandal' || levelWiseOverviewArr[i] == 'panchayat')
							{
								collapse+='<div class="col-sm-3">';
							...
							/* if(levelWiseOverviewArr[i] == 'panchayat')
							{
								collapse+='<div class="col-sm-3">';
									collapse+='<label>Constituency</label>';
									collapse+='<select class="chosen-select" id="'+levelWiseOverviewArr[i]+'SelectCons">';
	...
	for(var i in levelWiseOverviewArr)
	{
		...
		}else if(levelWiseOverviewArr[i] == "panchayat"){			
			getAllDistricts(levelWiseOverviewArr[i]+'SelectDist');
			
			getSolidInfoLocationWise(levelWiseOverviewArr[i]+'BodyId',0,0,"panchayat",startDate,endDate,0,"",0,"");
		}
	}
}
	...
	$.ajax({                
		...
		if(result != null)
		{
			if(distId ==0){
				...
				buildTable(result,blockid,locationId);
			}else if(distId>0){				
				for(var i in result){
			...
			table+='<table class="table table-bordered" id="'+blockid+'dataTableId" style="width:100%">';
				table+='<thead>';
					...
					if(isState){
							...
							table+='<th class="font_sz" colspan="7" style="text-align:center;">TODAY RFID ACHIEVED</th>';
					...
					table+='</tr>';
						...
						}else if(blockid == 'panchayatBodyId'){
							table+='<th rowspan="2" class="font_sz">PANCHAYATS</th>';
						}
							table+='<th class="font_sz" rowspan="2">TOTAL RFID TAGGED HOUSES</th>';
							table+='<th class="font_sz" colspan="7" style="text-align:center;">TODAY RFID ACHIEVED</th>';
							table+='<th class="font_sz" rowspan="2">REGISTERED FARMERS</th>';
				...
				if(isState){	
						...
						table+='<tr>';
							...
							if(achieve != null && achieve > 0){
								table+='<td style="background-color:#FFC0CB;">'+achieve+'</td>';
							}else{
								table+='<td style="background-color:#FFC0CB;">-</td>';
							}
							if(targetAchievePer != null && targetAchievePer > 0){
								table+='<td style="background-color:#FFC0CB;">'+targetAchievePer.toFixed(2)+'</td>';
							}else{
								table+='<td style="background-color:#FFC0CB;">-</td>';
							}
							if(inTime != null && inTime > 0){
								table+='<td style="background-color:#92DD5A;">'+inTime+'</td>';
							}else{
								table+='<td style="background-color:#92DD5A;">-</td>';
							}
							
							if(targetinTimePer != null && targetinTimePer > 0){
								table+='<td style="background-color:#92DD5A;">'+targetinTimePer.toFixed(2)+'</td>';
							}else{
								table+='<td style="background-color:#92DD5A;">-</td>';
							}
							if(outTime != null && outTime > 0){
								table+='<td style="background-color:#1076F1;">'+outTime+'</td>';
							}else{
								table+='<td style="background-color:#1076F1;">-</td>';
							}
							
							if(targetoutTimePer != null && targetoutTimePer > 0){
								table+='<td style="background-color:#1076F1;">'+targetoutTimePer.toFixed(2)+'</td>';
							}else{
								table+='<td style="background-color:#1076F1; color:#fff;">-</td>';
							}
							table+='<td>'+farmers+'</td>';
					...
					{			
						if(blockid == 'districtBodyId'){
							...
							if(blockid == 'panchayatBodyId'){
								table+='<td attr_dist_name="'+result[i].locationName+'" attr_dist_id="'+result[i].locationId+'" attr_onclick_distname="'+blockid+'" style="cursor: pointer" class="text-capital" data-toggle="modal" data-target="#swmModal">'+result[i].locationName+'<i class="fa fa-info-circle pull-right" aria-hidden="true" title="Click for '+result[i].locationName+' Details" attr_onclick_distname="'+blockid+'" attr_onclick_mandname="gpblockMand"  attr_dist_id="'+result[i].locationId+'" style="cursor: pointer"></i></td>';
							}else{
                           ...
                           //table+='<td style="background-color:#FFA600;">'+result[i].outOfTarget+'</td>';
							...
							if(result[i].achieve != null && result[i].achieve > 0){
								table+='<td style="background-color:#FFC0CB;">'+result[i].achieve+'</td>';
							}else{
								table+='<td style="background-color:#FFC0CB;">-</td>';
							}
							
							if(result[i].trackingPer != null && result[i].trackingPer> 0 ){
								table+='<td style="background-color:#FFC0CB;">'+result[i].trackingPer.toFixed(2)+'</td>';
							}else{
								table+='<td style="background-color:#FFC0CB;">-</td>';
							}
							if(result[i].inTime != null && result[i].inTime > 0){
								table+='<td style="background-color:#92DD5A;">'+result[i].inTime+'</td>';
							}else{
								table+='<td style="background-color:#92DD5A;">-</td>';
							}
							if(result[i].inTimePer != null && result[i].inTimePer > 0){
								table+='<td style="background-color:#92DD5A;">'+result[i].inTimePer.toFixed(2)+'</td>';
							}else{
								table+='<td style="background-color:#92DD5A;">-</td>';
							}
							if(result[i].outTime != null && result[i].outTime > 0){
								table+='<td style="background-color:#1076F1; color:#fff;">'+result[i].outTime+'</td>';
							}else{
								table+='<td style="background-color:#1076F1; color:#fff;">-</td>';
							}
							if(result[i].outTimePer != null && result[i].outTimePer > 0){
								table+='<td style="background-color:#1076F1; color:#fff;">'+result[i].outTimePer.toFixed(2)+'</td>';
							}else{
								table+='<td style="background-color:#1076F1; color:#fff;">-</td>';
							}
...
$(document).on('click','[attr_onclick_distname]',function(){
	...
	/* alert(distId);
	alert($(this).attr('attr_onclick_distname')); */
	
	var locIdClick = $(this).attr('attr_onclick_distname');
	var locationId = distId;
	//alert(locationId);
	//"1-06-2017","30-07-2017"
	if(distId != null && typeof distId != undefined && distId != 'undefined'){
		...
		}else if(locIdClick == 'panchayatBodyId'){
			$("#gpsCountBlockId").html('');	
			getSolidInfoLocationWise(0,distId,locationId,"panchayat","1-06-2017","30-07-2017",0,"",0,"");	
		}
		if($(this).attr('attr_onclick_mandname')=="gpblockMand" && (locIdClick == 'mandalBodyId' || locIdClick == 'panchayatBodyId'))
		{
...
$(document).on('change','#panchayatSelectDist',function(){
	var selmandalId = $(this).val();
	getAllConstituenciesForDistrict("panchayatSelectCons",selmandalId);
	 for(var i in levelWiseOverviewArr){
		if(levelWiseOverviewArr[i] == 'panchayat'){
			getSolidInfoLocationWise(levelWiseOverviewArr[i]+'BodyId',0,0,"panchayat","1-06-2017","30-07-2017",selmandalId,"district",0,"constituency");
		}
	} 
})
$(document).on('change','#panchayatSelectCons',function(){
	var selmandalId = $(this).val();
	for(var i in levelWiseOverviewArr){
		if(levelWiseOverviewArr[i] == 'panchayat'){
			getSolidInfoLocationWise(levelWiseOverviewArr[i]+'BodyId',0,0,"panchayat","1-06-2017","30-07-2017",0,"district",selmandalId,"constituency");
		}
	}
})
...
function buildGpWiseRfidTrackingOverData(result) {
	...
	for ( var i in result) {
		...
		str += '<h5 class="m_top5"><b>' + result[i].subList[0].name
				+ '- GP</b></h5>';
		str += '</div>';
		...
		str += '</div>';
		str += '<div class="col-sm-3">';
		str += '<h6><b>' + result[i].trackingPer.toFixed(2) + '</b></h6>';
		str += '</div>';
		str += '<div class="col-sm-3">';
		str += '<h6><b>' + result[i].inTimePer.toFixed(2) + '</b></h6>';
		str += '</div>';
		str += '<div class="col-sm-3">';
		str += '<h6><b>' + result[i].outTimePer.toFixed(2) + '</b></h6>';
		str += '</div>';
