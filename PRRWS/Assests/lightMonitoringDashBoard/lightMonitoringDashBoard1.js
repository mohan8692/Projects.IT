...
function onLoadCalls()
{
	...
	getDayWiseLedOverview();
	getCompanyWiseLightMonitoringDtls();
	menuWiseDetails();
	getLEDLocationWiseReport();
}
...
function projectData(divId,levelId)
{
									...
									collapse+='<div class="row m_top10">';
										...
										collapse+='<div class="col-sm-3">';
											collapse+='<ul class="nav navbar-nav list_inline tableMenu tableMenu'+dataArr[i]+'ViewType levelTypeCls" role="tabDrains_menu" attr_blockId="3">';
												collapse+='<li class="active viewTypeCls" attr_location_level='+dataArr[i]+' attr_tab_type="cummulative">Cummulative</li>';
												collapse+='<li class="viewTypeCls"  attr_location_level='+dataArr[i]+'  attr_tab_type="individual">Vendor</li>';
											collapse+='</ul>';
										collapse+='</div>';
										if(dataArr[i] == "district"){
	...
	for(var i in dataArr)
	{
		...
		getAllLevelWiseDataOverView(dataArr[i],dataArr[i],"",dataArr[i],lightVendorArr,"cummulative");
	}	
			 ...
			 if (result[i].subList != null && result[i].subList.length > 0) {
				  for(var j in result[i].subList) {
						...
						tableView+='<div class="dropup">';
							tableView+='<span class="pull-right dropdown-toggle tooltipWattCls" style="font-weight: 600; cursor: pointer; font-size: 12px;" id="dropdownMenu2" data-toggle="dropdown" aria-haspopup="true" aria-expanded="false" data-container="body" title="Wattage Details"><a>Wattage Details </a></span>';
										...
										for(var l in result[i].subList[j].wattageList){
											if (result[i].subList[j].wattageList[l].wattage !=0 && result[i].subList[j].wattageList[l].lightCount!=0){
												if(result[i].subList[j].wattageList[l].wattage == 24 || result[i].subList[j].wattageList[l].wattage == 32 || result[i].subList[j].wattageList[l].wattage == 75){
													tableView+='<li class="ledWattageColor"><b>'+result[i].subList[j].wattageList[l].wattage+'W = '+result[i].subList[j].wattageList[l].lightCount+'</b><span style="display:none;">,</span></li>';
												}else{
													tableView+='<li class=""><b>'+result[i].subList[j].wattageList[l].wattage+'W = '+result[i].subList[j].wattageList[l].lightCount+'</b><span style="display:none;">,</span></li>';
												}
											}
										}
									}else{
										tableView+='<li><b>0W = 0</b><span style="display:none;">,</span></li>';
									}
									tableView+='</ul>';
								tableView+='</div>';
								tableView+='</div>';
			...
			str+='<div class="col-sm-6 m_top10">';
				str+='<div class="pad_white_bg">';
					str+='<div class="media">';
					  str+='<div class="media-left">';
						str+='<h3>';
						  str+='<img class="media-object" src="Assests/icons/Essl.jpg" alt="Eesl" style="height:30px"/>';
						str+='</h3>';
					 str+='</div>';
					  str+='<div class="media-body">';
						str+='<h3 class="media-heading" style="color:#827C13">EESL</h3>';
					  str+='</div>';
					str+='</div>';
					...
					if(result.eeslVO.subList != null && result.eeslVO.subList.length > 0){
						...
						for(var k in result.eeslVO.subList){
							str+='<div class="col-sm-6 m_top10">';
								...
								if(result.eeslVO.subList[k].eeslnewCount != null && result.eeslVO.subList[k].eeslnewCount > 0)
									str+='<h3 class="font_weight m_top10">'+result.eeslVO.subList[k].eeslnewCount+'</h3>';
								else
									str+='<h3 class="font_weight m_top10">0</h3>';
							str+='</div>';
						}
						str+='</div>';
						str+='</div>';
			...
			str+='<div class="col-sm-6 m_top10">';
				str+='<div class="pad_white_bg">';
					str+='<div class="media">';
					  str+='<div class="media-left">';
						str+='<h3>';
						  str+='<img class="media-object" src="Assests/icons/Nredp.jpg" alt="Eesl" style="height:30px"/>';
						str+='</h3>';
					 str+='</div>';
					  str+='<div class="media-body">';
					...
					if(result.nredcapVO.subList != null && result.nredcapVO.subList.length > 0){
						...
						for(var k in result.nredcapVO.subList){
							str+='<div class="col-sm-6 m_top10">';
								...
								if(result.nredcapVO.subList[k].nredcapNewCount != null && result.nredcapVO.subList[k].nredcapNewCount > 0)
				...
				str+='<table class="table table_custom_block">';
					...
					if(result !=null){
						if(result.eeslVO !=null && result.eeslVO != 'undefined' && result.eeslVO.surveyStartedtotalDistricts !=0){
							...
							str+='<tr>';
								str+='<td><img class="media-object" src="Assests/icons/Essl.jpg" alt="Eesl" style="height:50px"/></td>';
								...
								str+='<td>';
									str+='<div class="dropdown">';
										...
										str+='<div class="dropdown-menu pull-right bg_ED arrow_box_bottom verticalScrollBar arrow_box_top_led" aria-labelledby="dropdownMenu3" style="margin-top: 32px;width: 1120px;">';
											str+='<div class="poles_block">';
												str+='<ul class="nav navbar-nav">';
													str+='<button type="button" class="close" data-dismiss="modal" aria-label="Close"><span aria-hidden="true">&times;</span></button>';
						...
						if(result.nredcapVO !=null && result.nredcapVO != 'undefined' && result.nredcapVO.surveyStartedtotalDistricts !=0){
							...
							str+='<tr>';
								str+='<td><h3><img class="media-object" src="Assests/icons/Nredp.jpg" alt="Eesl" style="height:50px;"></td>';
								...
								if(result.eeslVO.subList != null && result.eeslVO.subList.length > 0){
									for(var k in result.eeslVO.subList){
										if(result.eeslVO.subList[k].nredcapNewCount != null && result.eeslVO.subList[k].nredcapNewCount > 0)
								...
								str+='<td>';
									...
									str+='<span class="pull-right dropdown-toggle tooltipWattCls" style="font-weight: 600; cursor: pointer; font-size: 18px;" id="dropdownMenu4" data-toggle="dropdown" aria-haspopup="true" aria-expanded="false" data-container="body" title="Wattage Details">&#9432;</span>';
										str+='<div class="dropdown-menu pull-right bg_ED arrow_box_bottom verticalScrollBar arrow_box_top_led" aria-labelledby="dropdownMenu4" style="margin-top: 32px;width: 1200px;">';
											str+='<div class="poles_block">';
												str+='<ul class="nav navbar-nav">';
													str+='<button type="button" class="close" data-dismiss="modal" aria-label="Close"><span aria-hidden="true">&times;</span></button>';
...
function getRequiredTemplate(type){
		var str='';
			...
			if(type=="NREDCAP") {
				...
				str+='<div class="media">';
					  str+='<div class="media-left img_middle">';
						 str+='<img class="media-object" src="Assests/icons/Essl.jpg" alt="Eesl">';
			}
			 str+='</div>';
			  str+='<div class="media-body">';
				str+='<div class="col-sm-12">';
...
$(document).on("click","#ledLightsFilteredId",function(e){
		window.open("ledReport");
});

function getDayWiseLedOverview(){
	$("#dayWiseTargetsBlockId").html(spinner);
	var locationType="";
	var locationValue=0;
	if(globallevelId == 2 || globallevelId == 0){
		locationType = "";
	...
	$.ajax({                
		...
		beforeSend :   function(xhr){
	     ...
	     $("#dayWiseTargetsBlockId").html(' ');
		if (result != null) {
		   buildDayWiseLedOverviewDetails(result);	
		} else {
			$("#dayWiseTargetsBlockId").html("");
		}
...
//str+='<img src="Assests/icons/run_rate.png" alt="poles_icon">';

function buildDayWiseLedOverviewDetails(result)
{
	var str='';
	str+='<div class="row">';
		str+='<div class="col-sm-4">';
			str+='<div class="media">';
				str+='<div class="media-left">';
					str+='<img src="Assests/icons/run_rate.png" alt="poles_icon" style="width:25px;height:25px;">';
				str+='</div>';
				str+='<div class="media-body">';
					str+='<h5 style="color:#8E51Db;font-size:12px;"><b>REQ.&nbsp;RUN&nbsp;RATE/DAY</b></h5>';
					str+='<h4 class="m_top5">'+result.dayWiseTarget+'</h4>';
				str+='</div>';
			str+='</div>';
		...
		str+='</div>';
		str+='<div class="col-sm-4">';
			str+='<h5 style="color:#827C13;font-size:12px;"><b>TODAY FITTED</b></h5>';
			if(result.todayCount != null && result.todayCount > 0)
				str+='<h4 class="m_top5">'+result.todayCount+' <small style="color:#339900">'+result.todayPercentage+' %</small></h4>';
			else
				str+='<h4 class="m_top5">0 <small style="color:#339900">0.0 %</small></h4>';
		str+='</div>';
		str+='<div class="col-sm-4">';
			str+='<h5 style="color:#339900;font-size:12px;"><b>YESTERDAY FITTED</b></h5>';
			if(result.yeserdayCount != null && result.yeserdayCount > 0)
				str+='<h4 class="m_top5">'+result.yeserdayCount+' <small style="color:#339900">'+result.yesterdayPerc+' %</small></h4>';
			else
				str+='<h4 class="m_top5">0 <small style="color:#339900">0.0 %</small></h4>';
		str+='</div>';
	str+='</div>';
	...
	$("#dayWiseTargetsBlockId").html(str);
}
function getLEDLocationWiseReport(){
	$("#ledPDFReportSpinnerId").show();
	var json = {
		locationType:"state",
		locationId : 0
	}
	$.ajax({                
		...
		beforeSend :   function(xhr){
		 ...
		 $("#ledPDFReportSpinnerId").hide();	
	     if(result !=null){
			 globalResultsLEDReport=result;
			 $("#notLedPDFReportId").hide();
			 $("#ledPDFReportId").show();
		 }else{
			  $("#ledPDFReportId").hide();
			  $("#notLedPDFReportId").show();
		 }
