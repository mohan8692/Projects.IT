				...
				if(result[i].count !=null && result[i].count>0){
					str+='<div class="col-sm-3 m_top10">';
						str+='<table class="table table-bordered table-striped tableClr'+i+'">';
							str+='<thead>';
								str+='<tr>';
									if(result[i].name == "MGNREGS" && result[i].id == 0){
										str+='<th class="text-center" colspan="3"><h4>MGNREGS <span class="pull-right rankingColor">'+orderNo+'</span></h4></th>';
									}else{
							...
							str+='<tbody>';
								str+='<tr>';
									for(var j in result[i].subList)
									{
										...
										{
											if(result[i].subList[j].totl != null && parseFloat(result[i].subList[j].totl) > 0)
											{
												if(result[i].id == 0 && result[i].subList[j].name == "MATERIAL EXP"){
									...
									if(departmentId != 1 && departmentId != 3){
										...
										if(viewType == "cumulative"){
											if(result[i].count == 0) { // in the case MGNREGS we are sending count is zero and we are not displaying it.
											  table+='<td>'+Math.round(result[i].amount * 100)/100+'</td>';	
											}else {
								 ...
								 //location block
									if(levelId != '2'){
										if(result[i].subList[j].subList[k].count != null && result[i].subList[j].subList[k].count == 0){//in the case of MGNREGS count will come zero
											...
											table+='<td class="text-center no-right-border">'+Math.round(parseFloat(result[i].subList[j].subList[k].amount.replace(/,/g, "")) *100)/100+'</small></td>';
											...
											table+='<td><span class="text-center no-right-border fundSanctionCls toolltipCls" attr_scope_id="'+levelId+'" attr_level_value="'+lvlVal+'" attr_financial_yr_id="'+newYearId+'" attr_scheme_id="'+result[i].subList[j].subList[k].id+'" attr_dept_id="0" attr_location_name="'+locatioName+'" attr_level_name="'+levelName+'" attr_location_strids="'+result[i].locationIdStr+'" attr_mgnregs_name="'+result[i].subList[j].subList[k].name+'" attr_view_type="'+viewType+'" title="Click here to view total works ('+result[i].subList[j].subList[k].count+') details">'+Math.round(parseFloat(result[i].subList[j].subList[k].amount.replace(/,/g, ""))*100)/100+'</span></td>';
										}
									}else{
										if(result[i].subList[j].subList[k].count != null && result[i].subList[j].subList[k].count == 0){
											table+='<td class="text-center no-right-border">'+Math.round(parseFloat(result[i].subList[j].subList[k].amount.replace(/,/g, ""))*100)/100+'</td>';
										}else{
		...
		if(result.ttlAmt !=null && result.ttlAmt>0){
			...
			for(var i in result.subList)
			{
				if(result.subList[i].name == "MATERIAL EXP"){
					totalMGNMaterialCount = Math.round(result.subList[i].totl *100)/100;
				}
			}	
			if(schemes == "-1"){
				...
				total+='<h3>'+Math.round(parseFloat(result.totalAmt.replace(/,/g, "")) *100)/100+'</h3>';
				total+='<ul class="list-inline">';
				for(var i in result.subList)
				{
					total+='<li>';
