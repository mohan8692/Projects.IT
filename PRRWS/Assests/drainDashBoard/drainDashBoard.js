var globalFromDate = moment().subtract(10,'year').startOf("month").format('DD-MM-YYYY');
		...
		$.ajax({                
			...
			if(result != null){
				var totalStr='';
					...
					totalStr+='<span id="totalAvailableKmId">'+result.totalAvailableKms+'&nbsp;<small>KM</small></span>';
					totalStr+='</div>';
					totalStr+='<div class="col-sm-6 subBlock">';
					totalStr+='<h5>CLEANED</h5>';
					totalStr+='<h4 id="totalCleanedCountId">'+result.totalCleaned+'<small class="pull-right" id="totalPercentageId" style="color:#FD3367;">'+result.percentage+' %</small></h4>';
					totalStr+='<span id="totalCleanedKmId">'+result.totalCleanedKms+'&nbsp;<small>KM</small></span>';
					totalStr+='</div>';
					totalStr+='</div>';
				...
				var kachaStr='';
					...
					kachaStr+='<span id="kachaAvailableKmId">'+result.kachaAvailableKms+' <small>KM</small></span>';
					kachaStr+='</div>';
					kachaStr+='<div class="col-sm-6 subBlock">';
					kachaStr+='<h5>CLEANED</h5>';
					kachaStr+='<h4 id="kachaCleanedCountId">'+result.kachaCleaned+'<small class="pull-right" id="kachaPercentageId" style="color:#FD3367;">'+result.kachaPercentage+' %</small></h4>';
					kachaStr+='<span id="kachaCleanedKmId">'+result.kachaCleanedKM+' <small>KM</small></span>';
					kachaStr+='</div>';
					kachaStr+='</div>';
				...
				var pakkaStr='';
					...
					pakkaStr+='<span id="pakkaAvailableKmId">'+result.pakkaAvailableKms+' <small>KM</small></span>';
					pakkaStr+='</div>';
					pakkaStr+='<div class="col-sm-6 subBlock">';
					pakkaStr+='<h5>CLEANED</h5>';
					pakkaStr+='<h4 id="pakkaCleanedCountId">'+result.pakkaCleaned+'<small class="pull-right" id="pakkaPercentageId" style="color:#FD3367;">'+result.pakkaPercentage+' %</small></h4>';
					pakkaStr+='<span id="pakkaCleanedKmId">'+result.pakkaCleanedKM+' <small>KM</small></span>';
					pakkaStr+='</div>';
					pakkaStr+='</div>';
				...
				var ugStr='';
					...
					ugStr+='<span id="ugAvailableKmId">'+result.ugAvailableKms+' <small>KM</small></span>';
					ugStr+='</div>';
					ugStr+='<div class="col-sm-6 subBlock">';
					ugStr+='<h5>CLEANED</h5>';
					ugStr+='<h4 id="udCleanedCountId">'+result.ugCleaned+'<small class="pull-right" id="ugPercentageId" style="color:#FD3367;">'+result.ugPercentage+' %</small></h4>';
					ugStr+='<span id="ugCleanedKmId">'+result.ugCleanedKms+' <small>KM</small></span>';
					ugStr+='</div>';
					ugStr+='</div>';
					$("#ugBodyId").html(ugStr);
			}
				...
				for(var i in result){
					//if(result[i].totalAvailable !=null && result[i].totalAvailable>0){
						...
						if(result[i].totalCleanedKms !=null && result[i].totalCleanedKms>0){
							str+='<td style="background-color:#fff">'+result[i].totalCleanedKms.toFixed(2)+'</td>';
						}else{
							str+='<td style="background-color:#fff"> - </td>';
						}
...
function getTopImages(){
	...
	$.ajax({                
		...
		if(result != null){
			buildTopImages(result,"onload");
		}
	});
}
function buildTopImages(result,type){
	var str='';
	str+='<ul class="list-inline m_top10 imagesSliderCls'+type+'">';
	for(var i in result){
		str+='<li>';
			str+='<div class="thumbnail">';
			  str+='<img class= "m_top5 img-responsive onclickimg" onerror="setDefaultImage(this);" attr_type="'+type+'" attr_id="'+result[i].id+'" src="'+result[i].image+'"att_district="'+result[i].districtName+'" attr_mandal="'+result[i].mandalName+'" attr_division="'+result[i].divisionName+'" attr_panchayat="'+result[i].villageName+'"  attr_street="'+result[i].streetName+'"alt="" style="cursor:pointer; width:230px;height: 140px;" >';
				
				str+='<div class="caption">';
				if(result[i].streetName !=null && result[i].streetName.length>20){
					str+='<h5>Location :<span class="f_12 m_left_5 tooltipCls" style="cursor:pointer;" data-toggle="tooltip" data-placement="top" title="'+result[i].streetName+'">'+result[i].streetName.substring(0,20)+'...</span></h5>';
				}else{
					str+='<h5>Location :<span class="f_12 m_left_5 " style="cursor:pointer;">'+result[i].streetName+'</span></h5>';
				}
				  ...
				  str+='<h5 class="m_top5">Division :<span class="font_weight m_left_5" >'+result[i].divisionName+'</span></h5>';
				  str+='<h5 class="m_top5">Tehsil :<span class="font_weight m_left_5" >'+result[i].mandalName+'</span></h5>';
				  str+='<h5 class="m_top5">Panchayat :<span class="font_weight m_left_5" >'+result[i].villageName+'</span></h5>';
	...
	str+='</ul>';
	if(type =='onload'){
		$("#topImagesSlider").html(str);
	}else{
		$("#modalimageofVill").html(str);
	}	
	$('.imagesSliderCls'+type).slick({
		slide: 'li',
		slidesToShow: 5,
		slidesToScroll: 1,
		autoplay: true,
...
$(document).on("click",".onclickimg",function(){
	var id = $(this).attr("attr_id");
	var type = $(this).attr("attr_type");
	var district= $(this).attr("att_district");
	var division= $(this).attr("attr_division");
	var mandal= $(this).attr("attr_mandal");
	var village= $(this).attr("attr_panchayat");
	var street =$(this).attr("attr_street");
	$("#modalHablitationDivId").modal('show');
	
	$("#modalHabliHeadingId").html(village+" Village Drain Details");
	if(type=="onload"){
		
		getVillageDrains(id,"",village,mandal,district,street);
		getVillageDrains(id,"onchange", "","","",street);
	}else{
		getVillageDrains(id,"onchange", "","","",street);
	}
...
function getVillageDrains(id,calType,village,mandal,district,street){
	...
	var urL ='';
	if(calType !='onchange'){
		$("#drainVillID").html( "Drains of "+village+"(V)/"+mandal+"(M)/"+district+"(D)")
		$("#modalimageofVill").html(spinner);
		urL ='getImagesOnclickforVillage'
	}else{
		urL='getImagesOnclick'
		$("#onchangeImagesDivId").html(spinner);
	}
	$.ajax({                
		type:'POST',    
		url: urL,
		...
		if(result != null){
			if(calType =='onchange'){
				buildonchangeImages(result,id,street);
			}else{
				buildTopImages(result,"onclick");
			}
		}
	});
}

function buildonchangeImages(result,id,street){
	var str='';
	$("#onchangeHeadingId").html("("+street+") Drain Details");
	str+='<ul class="list-inline m_top10">';
	for(var i in result){
		str+='<li>';
			str+='<div class="thumbnail">';
			  str+='<img class= "m_top5 img-responsive" onerror="setDefaultImage(this);"  src="'+result[i].image+'" alt="" style="cursor:pointer; width:230px;height: 140px;" >';
				
				str+='<div class="caption">';
					str+='<h5>Status :<span class="font_weight m_left_5" >'+result[i].status+'</span></h5>';
					if(result[i].latitude !=null && result[i].latitude !='undefined'  && typeof(result[i].latitude) !=='undefined'){
						str+='<h5 class="m_top5">Latitude :<span class="font_weight m_left_5" >'+result[i].latitude+'</span></h5>';
					}
					if(result[i].longtitude !=null && result[i].longtitude !='undefined'  && typeof(result[i].longtitude) !=='undefined'){
						str+='<h5 class="m_top5">Longitude :<span class="font_weight m_left_5" >'+result[i].longtitude+'</span></h5>';
					}
				str+='</div>';
			str+='</div>';
		str+='</li>';
